...
protocol to generate a shared secret between a group of participants.
Each conversation participant is authenticated via a triple Diffie-Hellman
signing mechanism that occurs during group key agreement.
Individual header and message keys are updated after receipt of each message by incorporating
additional random key material distributed by the sending participant.
No security requirements are imposed on the underlying transport layer.
All setup operations prior to group key agreement can take place over an insecure
channel.
\end{abstract}
...
\mathrm{hash}(\mathrm{ECDH}(b_j, B_i) \, ||
\, \mathrm{ECDH}(r_j, R_i) \, || \, \mathrm{ECDH}(r_j, B_i))$ \\ and hmacs: \\
...
\mathrm{hash}(\mathrm{ECDH}(b_j, B_i) \, ||
\, \mathrm{ECDH}(r_j, R_i) \, || \, \mathrm{ECDH}(b_j, R_i))$ \\ and hmacs: \\
